...
Context: a single agent that parses intent, calls MCP tools, and emits a JSON chart spec. Not a coding agent — no sandboxing or filesystem editing needed.
...
| smolagents | Apache 2.0 | Most minimal (~1,000 lines). Code-as-action: agent writes Python instead of JSON tool calls — nice for composing MCP queries. |
...
- **Codex CLI** — OpenAI-first, sandbox-first. Built for repo editing, not tool-calling analytics loops.
...
OpenCode. It gives the Claude Code / Codex loop experience without model lock-in, and its MCP integration maps directly onto the existing MCP server that already enforces ACL and caching.
